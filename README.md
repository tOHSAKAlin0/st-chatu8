...
> D3 修复人物定义保存核对对 JSON 字段顺序敏感的问题，增加云端读取重试及具体失败原因。人物定义仍按相同编号保留参考窗口内最新记录。

> D3「人物定义保存与跨楼层参考」：默认参考前 3 个有效定义楼层，相同人物编号自动合并，只发送最后一层的完整定义；支持按 swipe 保存、查看和删除。当前版本 `2.8.4-definitions.3`；使用方法见 [USAGE.md](USAGE.md#人物定义参考d3)。下方 R2 信息为历史版本记录。

> 本修改版新增「历史绘图提示词参考」，配套独立 v3 Agent 预设。R2 修复界面缓存加载，并在标题和 LLM 页面显示「历史提示词版 R2」。使用与云酒馆安装方法见[安装与使用说明](USAGE.md)。

> 可选的[彩色无字漫画预设](presets/novelai-v5-story-comic-history.json)：基于 v3 历史参考，一图一页、自动 1–3 页、每页 2–4 格。导入与绑定见 [使用说明](USAGE.md#可选彩色无字漫画预设)。旧插图预设保留。


**作者:** 从前跟你一样
**基础版本:** 2.8.4（历史绘图提示词参考修改版）

**上游项目:** https://github.com/damoshen123/st-chatu8
