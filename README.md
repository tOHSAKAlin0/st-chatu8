...
> D4「视觉定义」：在人物定义之外新增场景与物件定义、常驻人物和编号自动校正；隐藏楼层不再使定义失效，单个条目不完整时只跳过该条；酒馆跳过保存时自动补存。当前版本 `2.8.4-definitions.4`；使用方法见 [USAGE.md](USAGE.md#视觉定义参考d4)。

> D1–D3 起按楼层及 swipe 保存定义：默认参考前 3 个有效定义楼层，相同编号只发送最后一层的完整定义，支持查看和删除。下方 R2 信息为历史版本记录。
